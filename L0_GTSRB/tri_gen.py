...
def mask_with_T0(trigger, mask):
    ...
    for h in range(32):
        for w in range(32):
            if mask[0, h, w] == 0:
                trigger.data[0, 0, h, w] = - ( 0.485 / 0.229 )
                trigger.data[0, 1, h, w] = - ( 0.456 / 0.224 )
                trigger.data[0, 2, h, w] = - ( 0.406 / 0.225 )
    return trigger
...
def clip_to_img(trigger, mask):
    ...
    for h in range(32):
        for w in range(32):
            if mask[0, h, w] == 0:
                if trigger.data[0, 0, h, w]  > ( 1.0 - 0.485) / 0.229:
                    trigger.data[0, 0, h, w] = ( 1.0 - 0.485) / 0.229
                if trigger.data[0, 0, h, w] < ( 0. - 0.485) / 0.229:
                    trigger.data[0, 0, h, w] = ( 0. - 0.485) / 0.229
                if trigger.data[0, 1, h, w]  > ( 1.0 - 0.456) / 0.224:
                    trigger.data[0, 1, h, w] = ( 1.0 - 0.456) / 0.224
                if trigger.data[0, 1, h, w] < ( 0. - 0.456) / 0.224:
                    trigger.data[0, 1, h, w] = ( 0. - 0.456) / 0.224
                if trigger.data[0, 2, h, w] > (1.0 - 0.406) / 0.225:
                    trigger.data[0, 2, h, w] = (1.0 - 0.406) / 0.225
                if trigger.data[0, 2, h, w] < (0. - 0.406) / 0.225:
                    trigger.data[0, 2, h, w] = (0. - 0.406) / 0.225
    return trigger
